...
- 契约版本：`2`
- 阶段：产品交付
- 定位：把已对齐 brief、已通过复审的原型和方案思考整理成产品设计文档或精简 PRD / 交付稿，并沉淀脱敏交付资产。
...
22. 如果用户要求产品设计文档，或上游已经完成三方案 image-2 原型图与复审，默认优先生成 `产品设计文档`；否则默认选择 `精简 PRD`。只有用户明确要求设计交付、实验验证或研发交付时，才追加对应补充，不把所有模板硬塞进一份文档。
23. 产品设计文档必须读取 prototype-board 中每个方案的原型思考、信息架构设计思考、用户问题解决逻辑、反指标保护和不可虚构边界，生成 `背景与现状 / 用户需求与证据 / 数据或访谈或截图或竞品启发 / 产品简述 / 三方案对比 / 每个方案原型图 / 信息架构设计思考 / 推荐方案 / 风险与待验证 / 下一步`。
24. 精简 PRD 核心只保留：需求背景、需求价值、需求方案、需求功能及描述、接口以及数据来源、埋点信息、实验标准。测试计划、开发周期、排期、人力、会议纪要和长风险清单默认不写。
...
28. 平台脚本可用时，用 `pmw-log handoff <name>` 保存交付稿，它会登记 `handoff` 到 Product Artifact Flow；产品设计文档额外登记 `product_design_doc`；需要给 QA、发布或文档同步接力时，额外用 `pmw-artifact add --kind acceptance_seed` 或 `--kind release_doc_seed` 记录下游可读摘要。记录 `pmw-run event --type artifact`，并且只有交付前门槛全部通过时才 `pmw-run finish --status "可交付"`。
