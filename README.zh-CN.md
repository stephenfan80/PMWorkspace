...
5. 新页面如果承接线上流程或生产样式，先完成线上参考检查。
6. `$pm-brief` 写出可确认、可复用的产品简报，并默认创建 Zoon 在线文档。
7. 产品简报达到“已对齐”后，`$pm-prototype-shotgun` 先读取 Zoon 最新内容，再生成 image-2 原型图。
8. `$pm-handoff` 把选定方向转成可交付文档。
