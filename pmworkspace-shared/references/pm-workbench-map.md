...
- `产品路径 / 执行深度 / 工作目标模式`：现在是在全新功能、已有功能迭代、快速成型还是深度交付。
...
- 硬门槛：产品路径不清、风险冲突或输入不足时必须停在路径/证据卡点；用户必须看见产品路径、执行深度、当前任务、当前主阻断和关键缺口队列。
- 输出：路由结果，进入最早缺失技能。
...
- 职责：写成短版产品简述 / 产品简报并固化出图前门槛，先讲清真实问题、PM 判断摘要、产品判断对抗校验、证据状态、目标用户、当前替代/损失、选中路径、范围模式、本周期验证、主目标、反指标、不可虚构项、原型范围、产品作业、待验证项、待确认信息与偏差风险。
...
- 输出：`product_brief` 登记到产物流动；如果仍有待确认信息，先让用户补齐或确认偏差风险；确认已对齐后再提醒是否同步到 Zoon，并进入三条产品路径原型或交付。
