...
- 快速成型模式面向新 idea、拍脑袋方案、先拿出去讨论的轻量包；默认按产品方向审查内核补齐最大缺口，列出关键假设、反指标和不可虚构项，用户确认“按这些假设继续”后，输出“产品简述 + 至少 3 条产品路径 + 原型图”轻量包。
- 深度交付模式面向 PRD、设计评审、研发交付、生产流程、Zoon 协作或高风险业务；原型相关请求默认按“产品路径 -> 工作目标模式 -> 产品方向审查内核 -> 产品简述确认 -> 三条产品路径 image-2 原型 -> 原型复审 -> 产品设计文档 / 产品交付”推进。
...
- 快速成型 / 10 分钟轻量包 / 简述 + 三条产品路径 + 原型图 -> `$pm-autoplan`
