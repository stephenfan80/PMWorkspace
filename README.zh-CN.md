...
2. 先确认工作目标模式：验证价值、优化线上指标、业务评审、设计评审或研发交付。
3. `$pm-jobs` 用 `Q` 一次一个问题诊断用户、现状替代、目标、反指标和约束。
4. 输出 2-4 条关键前提让 PM 确认；不同意就回到对应追问。
5. 关键 PM 决策用 `D` 选择题拍板，一次最多 3 个问题。
6. 新页面如果承接线上流程或生产样式，先完成线上参考检查。
7. `$pm-brief` 写出可确认、可复用的产品简报，并默认创建 Zoon 在线文档。
8. 产品简报达到“已对齐”后，`$pm-prototype-shotgun` 先读取 Zoon 最新内容，再生成 image-2 原型图。
9. `$pm-handoff` 把选定方向转成可交付文档。

## 原型输出规则

- 默认移动端优先：iPhone 17 竖屏 `402 x 874`。
- 只有用户明确要求桌面端，或看板 / 内部工具确实需要大屏密度，才使用桌面端。
- 产品简报没有“已对齐”前，不写图片提示词，不生成图片，不生成 HTML，不输出交付稿。
