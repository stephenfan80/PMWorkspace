...
PMWorkspace 的工作方式很简单：**先对齐，再出图，最后交付**。它不会把一句“帮我做个原型”直接变成图片，而是先确认真实问题、目标用户、当前替代、主目标、反指标、不可虚构项和必要证据；产品简报对齐后，再生成移动端优先的 image-2 原型图；原型复审通过后，再整理成产品设计文档、精简 PRD 或交付稿。
