...
- `复审输入`：图片路径或 URL、方案名、屏幕任务、产品假设、产品简报版本、Zoon 快照、线上参考、设计系统和 prototype-board item。
- `视觉审计`：有参考截图时记录 `visual_baseline`、参考图尺寸、目标输出像素、`pmw-image-audit` 结果和是否需要重出。
- `输出单元绑定`：每张图必须能追溯到一个 `方案 + 屏幕任务`，并绑定产品假设、用户行为假设、当前损失、牺牲项 / 后置项、主目标、反指标、不可虚构项和产品简报版本；缺绑定时先标 `需要补充参考`，不能凭视觉印象通过。
...
- `判定原因`：把问题归类为产品假设偏离、产品事实偏离、反指标风险、不可虚构违规、线上参考偏离、设计系统偏离、未决产品取舍或轻微偏好。
...
- 屏幕是否匹配已对齐的产品简报和场景路由？
- 屏幕是否忠实表达了该方案的产品假设、用户行为假设、当前损失和牺牲项？
...
- 每个方案都要有清楚的产品假设、用户行为假设、当前损失、牺牲项和取舍。
