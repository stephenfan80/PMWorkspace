...
本文件是 PMWorkspace 的全链路索引。它把 README 的产品叙事、`routing.md` 的下一技能选择、`pm-eval-system.md` 的 fixture 分类，以及各技能输出的状态字段统一到同一张表里。

本文件不维护第二套产品发现规则。产品发现 / 产品方向审查 / 产品作业卡的真源是 `product-office-hours.md`；发现门槛是真源 `product-discovery-gate.md`；线上截图和视觉基线门槛是真源 `production-reference-gate.md`。
...
PMW 项目信息很多，不能让每个 skill 从零拼装上下文。每次进入或切换技能时，先建立一个压缩的 `产品信息对齐包`，再执行本阶段动作。脚本可用时，优先读取 `pmw-dashboard status` 的默认摘要；需要审计时再展开 `pmw-dashboard status --details`、`pmw-artifact flow --details` 或 `pmw-discovery-gate check --target alignment --json`。

`产品信息对齐包` 是当前轮次的最小事实源，不是新增 artifact。它只整合已有 run、brief、Zoon、artifact-flow、discovery gate、视觉基线、prototype-board 和复审记录。

必须包含：

- `产品路径 / 对齐深度 / 工作目标模式`：产品路径是全新功能或已有功能迭代；对齐深度固定为深度产品对齐。
- `已知事实`：只写来自用户输入、截图、数据、Zoon、已对齐 brief、artifact 或当前 run 的事实。
- `产品信息对齐状态`：5 个核心事实维度是否覆盖：产品定位与链路角色、目标用户与触发场景、用户现状与当前替代、真实痛点与当前损失、主目标与反指标。
- `证据边界`：哪些是已确认事实，哪些仍是假设驱动，哪些需要用户补现实证据。
- `产品判断对抗校验状态`：是否已经指出想当然、理解偏差、证据不足和需要降级为假设的判断。
...
- 下游技能不能只依赖当前对话片段继续；必须先读取或重建 `产品信息对齐包`。
- 如果 `产品信息对齐包` 显示核心事实维度缺失，下一步只能是 `$pm-jobs` / `$pm-brief` / 证据请求，不得进入 image-2 prompt。
- 如果用户上传新截图、数据或 Zoon 修改，旧的 `产品信息对齐包` 立即过期；先更新事实和 brief 状态，再继续下游。
- `产品信息对齐包` 的用户可见版本展示当前判断、当前主阻断、关键缺口队列、产品作业卡和补齐后解锁；完整字段写入审计。

## 公开产品资产

PMWorkspace 对用户默认只解释 5 个核心产品资产；完整 artifact kind 仍由 `artifact-flow.md` 管理，并写入本地审计。

| 公开资产 | 对应阶段 | 用户看到的含义 |
|---|---|---|
| `product_brief` | 产品简报 | 已对齐事实源，决定能不能出图、复审和交付。 |
| `visual_baseline` | 线上参考 / 视觉基线 | 当前页面或视觉参考的尺寸、密度、保留区域和生成约束。 |
| `prototype_manifest` | 原型方案 | 每张图绑定的方案、屏幕、目标、反指标和不可虚构项。 |
| `prototype_review` | 原型复审 | 可通过、需要重出、补参考或 PM 拍板的结论。 |
| `handoff` | 产品交付 | 产品设计文档、精简 PRD、实验或研发交付稿。 |

`browser_evidence`、`repair_brief`、`product_design_doc`、`acceptance_seed` 和 `release_doc_seed` 是内部辅助资产；它们可以出现在审计、调试和维护文档里，但不作为欢迎页、README 或默认用户输出的第一层心智。

## 端到端链路

### 0. 欢迎、Operation Router 与产品路径路由 — `$pm-workspace`

- 职责：先用 `pmw-operation-router` 判断本轮是新产品任务、当前 gate 回答、补材料、修改 brief、原型局部修改、复审、重出还是交付续跑；只有新产品任务才继续判断 `全新功能` 或 `已有功能迭代`，再统一进入深度产品对齐，并创建 / 复用 run。
- 输入：用户任务、输入材料、风险信号、当前 run。
- 硬门槛：流程内 operation 不得重新 intake；产品路径不清、风险冲突或输入不足时必须停在路径/证据卡点；用户必须看见 operation、产品路径、对齐深度、当前任务、当前主阻断和关键缺口队列。
- 输出：operation 路由结果，进入最早缺失技能或最小续跑命令。
- 代表 eval：`pm-workspace-entry`、`pm-workspace-routing`、`pm-workspace-runtime`。

### 1. 自动产品评审 — `$pm-autoplan`

- 职责：以靠谱产品负责人姿态串联推荐链路，但只推进到最早可靠门槛。
- 输入：产品路径、用户目标、现有 brief / Zoon / run 证据。
- 硬门槛：产品路径冲突、产品发现深度不足、已有功能缺视觉基线、产品简报未对齐或 preflight 非 `ALLOW_IMAGE_PROMPT` 时必须停止；每轮必须告诉用户当前一步和下一步。
- 输出：当前最早门槛、产品作业卡、必要 Q/D，或带交接上下文交给下一个技能。
- 代表 eval：`autoplan`。

### 2. 产品方向审查内核 — `$pm-jobs`

- 职责：用强产品负责人姿态推进产品方向审查内核：前提挑战、现状替代、不做推演、路径对比、范围模式和当前拍板；每轮必须有当前判断、证据边界、建议范围模式、路径对比、PM 判断摘要、产品判断对抗校验和产品作业。
- 输入：产品定位、用户、触发时刻、现状替代、当前损失、目标、反指标、价值交换、约束、线上参考状态。
- 硬门槛：产品发现细则以 `product-office-hours.md` 和 `product-discovery-gate.md` 为准；已有功能迭代缺截图、关键节点截图或等价视觉基线时不能进入原型计划；不能用一个 Q + 一个 D 直接进入产品简述确认；范围变化必须进入一个当前 D。
- 输出：材料整理、证据请求、访谈/数据建议、PM 判断摘要、产品判断对抗校验、产品作业、路径对比、前提确认，或交给产品方向审查 / 产品简报。
- 代表 eval：`pm-jobs`。

### 3. 产品方向审查 — `$pm-strategy-review`

- 职责：先做前提挑战、现状替代、不做推演、路径对比和产品判断对抗校验，再用扩大、选择性扩大、保持、收缩或转向五种范围模式判断方向，并把 `最大策略矛盾` 转成 PM 取舍。
- 输入：`$pm-jobs` 交出的事实、风险门槛、反指标、不可虚构项。
- 硬门槛：基础事实缺失退回 `$pm-jobs`；进入范围模式前必须给 2-3 条路径并完成产品判断对抗校验；改变范围 / 承诺 / 实验 / 交付责任时只问一个 `D`；每个审查发现必须落成产品动作；不能省略范围模式。
...
- 职责：判断图片是否可通过、需要重出、需要 PM 拍板或补参考，先执行原型可信度对抗复审，避免把不可靠产品判断视觉化，并把用户反馈资产化。
- 输入：原型图片、输出单元绑定、brief、Zoon、线上参考、设计系统、设计规范目标、PM Review Army / Product Review Squad、可插拔复审专家、用户全局记忆、`prototype_manifest` 产物流动。
- 硬门槛：缺绑定不能凭视觉通过；策略、信任 / 风险、设计系统、数据可行性四个专家必须独立输出短结论；违反 brief / 反指标 / 不可虚构 / 线上参考 / 设计系统 / 10/10 原型标准或出现明显 AI 模板味时必须重出；原型把假设画成事实至少需要 PM 拍板，偏离真实问题、反指标冲突或画入不可虚构项时不能通过；产品承诺未决必须拍板；深度交付、高风险、批量交付、研发交付或多角色 review 必须输出 CEO、Eng、Design、DX、安全、QA、发布工程师短结论。
...
- `待确认`：已有假设、前提或推荐，需要用户确认；不能放行出图或交付。
...
| Eval 分类 | 对应阶段 | 主要守住什么 |
|---|---|---|
| `pm-workspace-entry`、`pm-workspace-routing`、`pm-workspace-runtime` | 欢迎与 D0 路由 | 欢迎页、产品路径、路由输出契约、run owner |
| `autoplan` | 自动产品评审 | 自动推进、深度对齐、最早门槛、终态 finish |
