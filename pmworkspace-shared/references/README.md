...
如果你只是想看 PMW 的产物长什么样，先读根目录 `examples/`。`examples/asset-flows/` 是公开样例库，展示新想法、已有功能、截图迭代和 PRD 交付如何从 `product_brief` 流到 `prototype_manifest`、`prototype_review` 和 `handoff`；本 `references/` 目录负责解释方法和运行协议，不承担公开样例库职责。
...
| 目录 | 定位 | 适合谁 |
|---|---|---|
| `examples/` | 公开资产样例库，展示产物链条长什么样。 | 第一次理解 PMW、演示、用户教育、手动验收。 |
| `pmworkspace-shared/references/` | 方法库和运行协议，解释为什么这么做、系统如何不跳步。 | Skill 作者、维护者、调试状态的人。 |
...
references 的目录结构最后才考虑移动。当前优先维护本索引，让读者先能分清“方法库”和“运行协议”；只有当索引已经无法解释职责边界、且移动成本明确低于长期混乱成本时，才调整文件位置。
