...
- 用户在对话或 Zoon 中调整产品简报后，必须重新保存并同步到 Zoon；原型或交付前做 Zoon 漂移检查。
...
- 原型复审默认使用 PM Review Army 四视角：策略、信任 / 风险、设计系统、数据可行性。
...
- 平台状态资产默认保存在 `~/.pmworkspace/`，只保存中文项目名、产品简报、选择题决策、Zoon 链接、原型清单、偏好反馈、脱敏产品学习和本地使用日志。
