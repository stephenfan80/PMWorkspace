...
- 新页面也要判断是否承接线上流程或生产样式；如果需要线上参考但缺失，先停下来问。
...
- 读取 `production-reference-gate.md`。
- 如果页面是线上流程的下一步、结果页、状态页、活动承接页，或需要匹配当前 App / Web 生产样式，就要求线上截图、录屏或相似页面参考。
- 用户明确确认没有线上参考时，可以按概念稿推进，但要在产品简报里标注 `线上参考状态：无线上参考已确认`。
- `线上参考需求：需要` 且 `线上参考状态：缺失待补充` 时，不能写图片提示词或生成原型图。
