...
- **产品简述 / 产品简报：** 出图前先输出短版对齐物，讲清真实问题、证据状态、目标用户、当前替代 / 损失、选中路径、范围模式、本周期验证、主目标、反指标、不可虚构项、原型范围、待验证项、待确认信息与偏差风险。完整 Q/D、来源、准备度、产物流动和路径进入本地审计。
...
- **产品作业卡：** 每轮先把已知事实、证据边界、5 个核心事实维度、当前主阻断、关键缺口队列、PMW 产品建议和信息架构建议收束到同一处，避免在分散材料里误判已对齐。
...
4. **本地保存，已对齐后按需同步 AI 协作文档。** 产品简报阶段默认先保存本地 Markdown；如果还有待确认信息，会先告诉你缺什么、不补齐可能导致什么产品方案偏差，以及本轮怎么补齐。只有产品简报确认状态已对齐后，才推荐 Zoon 协作价值并让用户 A/B 选择；只有用户选择同步、提供 Zoon URL，或任务明确需要在线协作且简报已对齐时，才创建或追加到 Zoon。已启用 Zoon 后，后续出图和交付前会检查是否发生漂移。
...
PMWorkspace 第一次给你的价值不是“更快画图”，而是先帮你获得一个清晰的产品判断：这是不是正确问题、当前损失是否真实、应该走哪条产品路径、哪些证据不足，以及补齐后能不能进入可信原型或交付稿。

四类入口可以这样开始：

- **一句 idea：** 先拿到真实问题暂判、当前替代、最小路径和一个现实产品作业，再决定是否进入产品简述和三条产品路径。
- **已有功能优化：** 先补线上截图 / 关键节点截图和现状数据，判断问题区域、保留项 / 可改项和原型可信度，避免把错误问题画得更精致。
- **要原型：** 先确认 brief、线上参考、方案差异、设计规范目标和不可虚构项；准备度通过后再逐张生成 image-2。
- **要 PRD / 交付稿：** 先检查已选方向、复审状态和研发能力依赖，避免把未具备的数据、接口、算法或合规能力写成验收承诺。
...
- 新页面如果承接线上流程、结果页、状态页或生产样式，必须先提供截图 / 关键节点截图 / 相似页面，或明确确认“没有线上参考，按新页面概念稿推进”。
