...
- Generated prototypes inherit canvas mode from `image-prompts.md`: no screenshot baseline uses the requested mobile canvas; screenshot baseline uses physical long-board output.
- For Autohome production-page prototypes with a screenshot reference, follow `prototype-shotgun-board.md` for `canvas_mode=physical_longboard` and `target_output_pixels`; keep AutoDesign spacing logic without repeating the full long-board protocol here.
...
- Canvas and pixel output: use `image-prompts.md` as the image-2 prompt truth source and `prototype-shotgun-board.md` as the output-unit truth source. With screenshot reference, the output unit must use physical long-board mode and target output pixels; do not repeat or loosen that protocol in this AutoDesign profile.
