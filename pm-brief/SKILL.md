...
12. 先建立产品简报契约控制器，记录 `来源门槛`、`已完成门槛`、`缺失门槛`、`事实/假设边界`、`策略决策写入`、`简报深度`、`下一技能` 和 `证据状态`。
...
17. 根据模糊程度、风险等级和证据状态选择快速版、标准版或深度版产品简报。
18. 包含事实来源、版本、确认状态、来源门槛、简报深度、缺失门槛、事实/假设边界、工作目标模式、目标人群、核心问题梳理、已确认前提、场景路由、页面类型、线上参考需求、线上参考状态、互联网案例检索、用户任务、目标、反指标、约束、原型内容重点、不可虚构项和 PM 决策项。
19. 把已拍板的策略取舍写入对抗审查后的设计取舍、方案方向、范围外、决策记录和对原型的影响；未拍板的策略取舍只能放进当前 D 或后续 D 队列。
20. 已对齐且用户要原型时，下一技能是 `$pm-prototype-shotgun`；已对齐且用户要交付时，下一技能是 `$pm-handoff`；未对齐时停在 `$pm-brief` 或回到上游缺失门槛。
21. 从功能名或产品简报标题提炼中文项目名，并用 `pmw-project set-name "<中文项目名>"` 保存。
22. Save the brief with `pmw-log brief <name>` when platform scripts are available. This automatically creates or appends the brief to Zoon when `zoon_sync_on_brief` is enabled.
23. Create or update the Zoon online brief by default:
