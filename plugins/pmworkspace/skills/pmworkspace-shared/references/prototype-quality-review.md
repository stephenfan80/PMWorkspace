...
- `原型可信度对抗复审`：判断图片是否把不可靠产品判断视觉化，是否把假设画成事实，是否让用户误解产品能力，是否偏离真实问题、当前替代、当前损失、反指标或不可虚构边界。
...
- 屏幕是否解决真实问题，还是只是把用户最初的功能愿望画得更完整？
- 屏幕是否把未经验证的用户行为、转化提升、推荐准确性、真实数据、权益资格、价格或结果承诺画成确定事实？
- 屏幕是否放大了产品评估阶段已标记的想当然、理解偏差、证据不足或需要降级为假设的判断？
...
原型复审必须先判断图片的产品可信度，再进入“是否好看 / 是否完整”的判断。漂亮但误导用户、误导团队或误导交付的图片不能通过。

每张图都要检查并记录：

- `真实问题一致性`：原型是否仍在解决产品简报里的真实问题，而不是偏到次要功能或内部目标。
- `假设视觉化风险`：是否把假设驱动、待验证或证据不足的判断画成已验证事实。
- `反指标冲突`：是否为了主目标牺牲信任、质量、投诉、留存、任务成功率、成本或合规护栏。
- `不可虚构违规`：是否画入未支持数据、能力、承诺、按钮、推荐、库存、价格、资格或结果。
- `理解偏差风险`：原型是否可能强化了 PMW 对用户诉求的错误理解。
- `用户能力误解`：用户看完是否会以为产品已经具备某个尚未确认的能力。

判定规则：

- 发现假设被画成事实，至少标为 `需要 PM 拍板`；如果画面已经明确承诺无法兑现的能力，标为 `需要重出`。
- 发现反指标冲突、不可虚构项被画入、或原型偏离真实问题，不能 `可通过`。
- 如果缺少证据导致无法判断原型可信度，标为 `需要补充参考`，不要凭视觉印象通过。
- 对抗复审结论必须进入 `prototype_review` 产物摘要，供 `$pm-handoff` 生成产品设计文档时读取。
