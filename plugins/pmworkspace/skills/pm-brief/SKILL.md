...
创建后续三条产品路径原型、复审、产品设计文档、交付稿和实验步骤都必须读取的产品简述 / 产品简报。它不是会议纪要，也不是最终产品设计文档；用户可见版必须是决策型短简报，先讲清一句话判断、推荐方案、为什么这样做、原型范围、不能虚构 / 不能承诺和当前需要确认。完整事实、D 记录、证据边界、状态门槛、产品作业卡、PM 判断摘要和产物流动继续写入本地审计副本。
...
7. Read `../pmworkspace-shared/references/runtime-kernel.md`; follow its Operation Router + Run Owner 协议 through `pmw-trigger-guard` / `pmw-operation-router` / `pmw-controller`. If called directly, first classify the user move with `pmw-operation-router classify`; only `operation_type=new_product_workflow` may run `pmw-controller intake --goal "<本轮产品简报目标>" --materials "<本轮用户材料摘要>" --skill pm-brief --product-path "<全新功能|已有功能迭代>" --depth "deep" --stage brief`. For `answer_pending_gate`、`lock_or_confirm_brief`、`modify_brief` or other flow-internal operations, execute the router-specified minimal command and inherit the current context instead of creating a new intake. Then run `pmw-controller preflight --target brief --json` when the operation target is brief readiness.
...
19. 用户可见输出必须先给业务版 `产品简述 / 产品简报`，默认只包含 `一句话判断`、`推荐方案`、`为什么这样做`、`原型范围`、`不能虚构 / 不能承诺`、`当前需要确认` 和必要的 `下一步`。用户可见简报不是审计日志；不得默认逐节输出 `产品作业卡`、`PM 判断摘要`、`产品判断对抗校验`、`证据状态`、`背景与现状`、`目标用户`、`当前替代 / 损失`、`主目标与反指标`、`选中路径与范围模式`。
    - Zoon、线上参考、检索来源、已保存资产和证据边界默认进入内部审计；产品作业卡、PM 判断摘要、对抗校验、Q/D 记录和产物流动也默认进入内部审计；业务正文只保留会影响用户确认的结论、取舍、范围、边界和待确认问题。
20. `产品核心信息` 可以作为内部摘要保留，但不能替代用户可见短版产品简报；真实问题、证据状态、用户痛点、当前替代/损失、选中路径、范围模式、本周期验证和待验证项要被压缩进 `一句话判断`、`推荐方案`、`为什么这样做`、`原型范围` 和 `当前需要确认`。
...
Return the smallest useful product-manager brief. 用户可见第一屏只放业务简报，不放工作流字段、仪表盘表格、产物流动或本地路径；默认输出短版 `产品简报`、逐项 `当前需要确认` 和必要的 `下一步`，不得输出 `产品简报门槛`、`支持信息`、run_id、Zoon 状态、产物流动或已保存资产，也不得把审计字段堆成正文：
