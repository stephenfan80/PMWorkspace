...
- 契约版本：`2`
- 阶段：产品交付
- 定位：把已对齐 brief 和已通过复审的原型压缩成精简 PRD / 交付稿，并沉淀脱敏交付资产。
...
18. 建立交付控制器，记录 `交付目标`、`事实来源`、`Product Readiness Dashboard`、`交付前门槛`、`原型复审状态`、`未决拍板`、`交付类型`、`PRD 缺口处理`、`验收写入边界`、`交付资产沉淀`、`上游产物`、`本轮产物`、`下游可读`、`产物流动`、`下一技能` 和 `证据状态`；这些默认写入审计，不能进入 PRD 正文。用户可见输出只保留交付结论、精简 PRD、待补充项和下一步，不输出 `PRD 生成判断`。
...
24. 缺接口、数据字段、埋点属性或实验细节但不改变承诺时，保留空值并写入 `待补充项`；每项状态使用 `待补充`、`已跳过` 或 `已补充`，用户可回复 `跳过某项` 或 `全部先跳过`，普通缺口不阻断当前 PRD。不能虚构字段、口径、事件名、接口可用性或实验阈值。
