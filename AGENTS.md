...
- 快速成型模式面向新 idea、拍脑袋方案、先拿出去讨论的轻量包；默认用 2-3 个关键 Q 补齐最大缺口，列出关键假设、反指标和不可虚构项，用户确认“按这些假设继续”后，输出“产品简报 + 方案方向 + 原型图”轻量包。
- 深度交付模式面向 PRD、设计评审、研发交付、生产流程、Zoon 协作或高风险业务；原型相关请求默认按“工作目标模式 -> 场景路由 -> Q 诊断 -> 前提确认 -> D 拍板 -> 产品简报 -> image-2 原型”推进。
- 每次进入或切换工作流时，必须给用户一个短的“工作方式卡片”：当前是 10min 快速成型还是深度交付、为什么这样判定、当前卡点、下一步只需要用户做什么；不能只在内部审计里记录模式。
...
- 诊断问题使用 `Q`，一次只问一个并等待用户回答；固定的是诊断维度，不是问题文本，每轮根据用户输入和当前最大缺口动态生成 Q，通常 2-3 个，最多 5 个；拍板问题使用 `D`，信息足够后一次只展开一个完整选择题。
...
- 默认原型输出移动端优先：无线上截图时使用 iPhone 17 竖屏 `402 x 874`；有生产截图 / `visual_baseline` 时必须使用截图物理像素长板，`402 x 874` 只能作为逻辑参考，不能作为 image-2 输出画布。
...
- 原型图生成和复审后必须给出反馈入口和下一步引导：选择一个方案继续、指出要改的字段/区域、继续探索新方案、补充截图/数据参考，或进入 `$pm-handoff` 生成精简 PRD / 交付稿。
- 用户对原型提出修改时，先判断是“设计改动”“产品承诺变化”还是“交付范围变化”；设计改动可直接重出受影响单图，产品承诺或范围变化必须回写产品简报并重新确认。
- 产品交付默认输出精简 PRD，只保留需求背景、需求价值、需求方案、需求功能及描述、接口以及数据来源、埋点信息、实验标准、待补充项和不可虚构；测试计划、开发周期、排期和人力默认不写。
...
- 快速成型 / 10 分钟轻量包 / 简报 + 方案方向 + 原型图 -> `$pm-autoplan`
