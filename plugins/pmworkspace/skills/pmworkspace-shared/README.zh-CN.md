...
- **产品简述 / 产品简报：** 出图前先输出短版对齐物，讲清真实问题、证据状态、目标用户、当前替代 / 损失、主目标、反指标、不可虚构项、方案机会、原型范围和待验证项。完整 Q/D、来源、准备度、产物流动和路径进入本地审计。
...
PMWorkspace 会按产品发现任务流补齐最大缺口，必要时协助整理材料、生成访谈提纲、梳理数据口径或做最佳实践摘要。你确认“按这些假设继续”后，它会输出一套轻量包：
...
2. **再跑产品发现任务流。** 它会协助做证据收集、用户需求澄清、数据 / 现状佐证、方案机会判断和产品简述确认，而不是只问一个问题就拍脑袋出方案。
...
- **新想法验证：** 从一句 idea 生成轻量产品简述、三方案方向和讨论用原型图。
...
| `$pm-jobs` | 按产品发现任务流澄清全新功能或已有功能迭代，补齐证据、需求、现状和方案机会。 |
